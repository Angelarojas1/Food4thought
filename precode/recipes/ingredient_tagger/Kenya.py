...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Kenya.csv")
data.shape
...
data['List of ingredients'].head(20)[1]
...
def ingredientTagger(ingredientStr):
    ...
    if ")" == ingredientStr[-1] and "(" in ingredientStr:
        temp = ingredientStr.split("(")[1].replace(")","")
        result["qty"] = re.findall(r"[-+]?(?:\d*\.\d+|\d+)", temp)[0]
        result["unit"] = (re.sub(r"[-+]?(?:\d*\.\d+|\d+)", '', temp)).split()[0]
    ...
    # step 4: if a digit is in the ingredient 
        ...
        if "of" in ingredientStr.split():
            ...
            if num_there(ingredientStr.split("of")[0]):
                result["item"] = (ingredientStr.split("of")[1]).split(')')[1]
                ...
                if ifUnit(ingredientStr)[0] == True:
                    result["unit"] = ingredientStr.split()[ifUnit(ingredientStr)[1]]
                    result["item"] = (' '.join(ingredientStr.split()[ifUnit(ingredientStr)[1]+1:])).split(')')[1]
                    ...
                    result["qty"] = re.findall(r"[-+]?(?:\d*\.\d+|\d+)", ingredientStr)[0]
                    result["item"] = (re.sub(r"[-+]?(?:\d*\.\d+|\d+)", '', ingredientStr)).split(')')[1]
        ...
        # step 5: and if “of” isn't in the ingredient     
            ...
            # step 5.2: if we don't have a unit in the ingredient
                ...
                result["item"] = (' '.join((re.sub(r"[-+]?(?:\d*\.\d+|\d+)", '', ingredientStr)).split()[1:])).split(')')[1]
...
data['Ingredient list tagger'].head(20)[17]
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Kenya.csv")
