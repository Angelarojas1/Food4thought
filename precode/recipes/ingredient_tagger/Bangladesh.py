...
# #### Ingredient Tagger of Bangladesh
...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Bangladesh.csv")
data.shape
...
data['List of ingredients'] = data['List of ingredients'].apply(lambda x: x.replace( '\\xa0', ' '))
...
data['List of ingredients'] = data['List of ingredients'].apply(lambda x: x.replace("½",'0.5').replace('¼','0.25').replace('⅓','0.3333').replace("'",""))
...
data['List of ingredients'].head(40)[10]
...
data['Ingredient list tagger'].head(20)[8]
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Bangladesh.csv")
