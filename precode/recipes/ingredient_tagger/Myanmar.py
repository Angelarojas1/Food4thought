...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Myanmar.csv")
data.shape
...
data['List of ingredients'] = data['List of ingredients'].apply(lambda x: x.replace('[[','').replace(']]',''))
...
def ingredientTagger(ingredientStr):
    ...
    result["input"] = ingredientStr.replace("'","").replace(',','',1)
    
    # step 0: transfer ingredient to lower case
    ingredientStr = (ingredientStr.lower()).replace("'","").replace(',','',1)
    ...
    # step 4: if a digit is in the ingredient 
        ...
        # step 5: and if “of” isn't in the ingredient     
            ...
            # step 5.2: if we don't have a unit in the ingredient
                ...
                result["item"] = ' '.join((re.sub(r"[-+]?(?:\d*\.\d+|\d+)", '', ingredientStr)).split()[0:])
...
data['Ingredient list tagger'].head(50)[22]
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Myanmar.csv")
