...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Albania.csv")
...
def ingredientTagger(ingredientStr):
    ...
    # step 0: if we have : in  the ingredient, split by : and get item 
    if ":" in ingredientStr:
      result["item"] = ingredientStr.split(":")[0]
      ingredientStr = ingredientStr.split(":")[1]
        
      if num_there(list(ingredientStr.split(" ")[0])[-1])==False:
        new_char = seperate_string_number(ingredientStr.split(" ")[0])
        new_char = ' '.join(new_char)
        ingredientStr = ingredientStr.replace(ingredientStr.split(" ")[0], new_char)
...
def AlbaniaIngredientTagger(ingredientLst):
    """
    input: ingredient string in English, like "carrots: 1 pound (young ones if possible)"
    output: dictionary like
...
def IngredientTagger(ingredientLst):
    ...
    for i in ingredientLst:
        try:
            result.append(AlbaniaIngredientTagger(i))
...
data['Ingredient list tagger'].head(20)[3]
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Albania.csv")
