...
# #### Ingredient Tagger of Afghanistan
...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Afghanistan.csv")
#data.drop(['Unnamed: 0'],axis=1,inplace=True)
data['List of ingredients'].head(10)
...
for key in ['sugar','egg','yolk','flour','baking powder','milk','butter']:
    data[key] = data['List of ingredients'].apply(lambda x: True if sum([key in i.lower() for i in x])>=1 else False)
...
def sugarAmount(ingredientLstTagger):
    ...
    unitMeasure = pd.ExcelFile("/content/drive/MyDrive/DATA/Unit standard.xlsx")
...
data['Ingredient list tagger'].tail(20)[70]
...
def calProportion(ingredientLstTagger):
    ...
    # import fruit list
    import json
    with open("/content/drive/MyDrive/DATA/fruits.json") as read_file:
...
data[['List of ingredients','Ingredient list tagger']].head(9)
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Afghanistan.csv")
