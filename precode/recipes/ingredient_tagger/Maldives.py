...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Maldives.csv")
data.shape
...
data['List of ingredients'].head(80)[79]
...
def ingredientTagger(ingredientStr):
    ...
    result["input"] = ingredientStr.replace('\r','')
    
    # step 0: transfer ingredient to lower case
    ingredientStr = ingredientStr.lower().replace('\r','').replace('–',',')
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Maldives.csv")
