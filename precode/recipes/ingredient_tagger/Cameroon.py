...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Cameroon.csv")
data.shape
...
data['List of ingredients'].head(10)[3]
...
data['List of ingredients'].head()[1]
...
def ingredientTagger(ingredientStr):
    ...
    result["input"] = ingredientStr.replace('\\n ','')
    
    # step 0: transfer ingredient to lower case
    ingredientStr = ingredientStr.lower().replace('\\n ','').replace('\\n','')
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Cameroon.csv")
