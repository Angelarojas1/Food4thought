...
# #### Ingredient Tagger of Indonesia
...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Indonesia.csv")
data.shape
...
data['List of ingredients'].head(500)[11]
...
def IndoneasiaIngredientTagger(ingredientLst):
    """
...
def IngredientTagger(ingredientLst):
    ...
    for i in ingredientLst:
        try:
            result.append(IndoneasiaIngredientTagger(i))
...
data['Ingredient list tagger'] = data["List of ingredients_Eng"].apply(lambda x: IngredientTagger(x))


# In[17]:


data['Ingredient list tagger'].head(2000)[1200]
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Indonesia.csv")
