...
# import dataset
data1 = pd.read_csv("/content/drive/MyDrive/DATA/Ukraine.csv")
data2 = pd.read_csv("/content/drive/MyDrive/DATA/Ukraine.csv")
...
data1['List of ingredients'] = data2['List of ingredients'].apply(lambda x: x.replace("'","").replace(':,',''))
...
data2['List of ingredients'] = data2['List of ingredients'].apply(lambda x: x.replace("'","").replace(':,','').replace('–','-'))
...
data1['List of ingredients'].head(30)[12]
...
data['List of ingredients'].tail()[299]
...
def ingredientTagger(ingredientStr):
    ...
    ingredientStr  = (ingredientStr.lower()).replace(']','').replace('[','') 
    result["input"] =  ingredientStr
    
    if "-" in  ingredientStr: 
      result['item'] = ingredientStr.split('-', 1)[0]
      ingredientStr = ingredientStr.split('-',1)[1] 
      ...
      if ")" == ingredientStr[-1] and "(" in ingredientStr:
          ...
          ingredientStr = (ingredientStr.split("(")[0]).replace(',','')
...
data['Ingredient list tagger'].tail()
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Ukraine.csv")
