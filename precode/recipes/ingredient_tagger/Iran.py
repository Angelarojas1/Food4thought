...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Iran.csv")
data.shape
...
def IranIngredientTagger(ingredientLst):
    """
...
def IngredientTagger(ingredientLst):
    ...
    for i in ingredientLst:
        try:
            result.append(IranIngredientTagger(i))
...
data['Ingredient list tagger'].head(40)[29]
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Iran.csv")
