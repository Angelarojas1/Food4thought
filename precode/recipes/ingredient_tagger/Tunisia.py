...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Tunisia.csv")
data.shape
...
def ingredientTagger(ingredientStr):
    ...
    if ")" == ingredientStr[-1] and "(" in ingredientStr:
        result["preparation"] = (re.search( "^.*\((.*)\).*" ,ingredientStr)).groups()[0]
...
data['Ingredient list tagger'].head(30)[11]
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Tunisia.csv")
