...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/Fiji.csv")
...
data['List of ingredients'].head(50)[35]
...
def ingredientTagger(ingredientStr):
    ...
    # if "–" in ingredientStr and num_there(list(ingredientStr.rsplit('–')[-1])[1])== False:
    #     result["preparation"] = ingredientStr.rsplit("–")[-1]
    #     ingredientStr = ingredientStr.rsplit("–")[0]
    # if "–" in ingredientStr and num_there(list(ingredientStr.rsplit('–')[-1])[1])== True:
...
data['Ingredient list tagger'].head(50)[39]
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/Fiji.csv")
