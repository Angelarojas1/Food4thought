...
type(data['List of ingredients'][0])
...
data['List of ingredients_Eng'].head(7000)[5]
...
def ingredientTagger(ingredientStr):
    ...
    if '-' in ingredientStr: 
        result['preparation'] = ingredientStr.split('-', 1)[0]
        ...
        if ")" == ingredientStr[-1] and "(" in ingredientStr:
            result["item"] = ingredientStr.split("(")[1].replace(")","")
        ...
        if "," in ingredientStr:
            ...
            # step 1.2: if the second part doesn't contain digit, then get the optional preparation method
                ...
                result["item"] = ingredientStr.split(",")[1]
                ingredientStr = ingredientStr.split(",")[0]
...
data['Ingredient list tagger'].head(9000)[6770]
