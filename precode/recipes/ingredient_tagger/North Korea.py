...
# #### Ingredient Tagger of North Korea
...
# import dataset
data = pd.read_csv("/content/drive/MyDrive/DATA/North Korea.csv")
data.shape
...
def ingredientTagger(ingredientStr):
    ...
    if "/" in ingredientStr:
        ...
        if len(list(ingredientStr.split()[0]))==1:
          temp =  int(list(ingredientStr.split('/')[0])[0]) + (int(list(ingredientStr.split('/')[0])[3])/int(list(ingredientStr.split('/')[1])[0]))
...
data['Ingredient list tagger'].head(30)[1]
...
data.to_csv("/content/drive/MyDrive/DATA/INTERMEDIATE/North Korea.csv")
