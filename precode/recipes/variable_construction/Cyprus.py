...
# #### Variable constructions of Cyprus
...
# import dataset
data = pd.read_csv("/Users/xixi/Dropbox/food4thought/data/final/Cyprus.csv")
...
def getTime(string):
    ...
    stringLst = [float(s) for s in re.findall(r'[-+]?(?:\d*\.\d+|\d+)', string)]
    
    if len(stringLst) == 2:
        stringLst = stringLst[1:]
    
    if len(stringLst) == 1:
        if "minutes" in string:
            return stringLst[0]
        elif ("hours" in string) | ("hrs" in string) | ("hour" in string):
            return stringLst[0]*60     
        elif "days" in string:
            return stringLst[0]*60*24
...
data.to_csv("/Users/xixi/Dropbox/food4thought/data/final/Cyprus.csv")
