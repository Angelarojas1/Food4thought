...
# ### Variable constructions for Zimbabwe
...
# import dataset
data = pd.read_csv("/Users/xixi/Dropbox/food4thought/data/intermediate/Zimbabwe.csv")
...
def getTime(string):
    if isinstance(string, str):
        ...
        if len(digits) == 0:
            return float("nan")
        if "hr" not in string or "hours" not in string:
            return digits[0]
        else: 
            if "min" in string:
...
data['Total time'] = data['Cook time'].apply(lambda x: getTime(x))
...
# replace those with nan values
data['Total time'] = data['Total time'].replace(0,np.nan)
...
data.to_csv("/Users/xixi/Dropbox/food4thought/data/final/Zimbabwe.csv")
