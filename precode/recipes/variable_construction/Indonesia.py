...
# #### Variable constructions of Indonesia
...
# import dataset
data = pd.read_csv("/Users/xixi/Dropbox/food4thought/data/final/Indonesia.csv")
...
# replace nan with 0
data['Total time'] = data['Total time'].fillna('')

# extract numbers from time columns
data['Total time lst'] = data['Total time'].apply(lambda x: [int(s) for s in x.split() if s.isdigit()])


# In[6]:


def calTime(Time_Lst):
    """
    input: time list
...
data['Total time'] = data['Total time lst'].apply(lambda x: calTime(x))
...
data.to_csv("/Users/xixi/Dropbox/food4thought/data/final/Indonesia.csv")
