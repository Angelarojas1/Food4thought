...
# ### Variable constructions for China
...
# import dataset
data = pd.read_csv("/Users/xixi/Dropbox/food4thought/data/intermediate/China.csv")
...
# transform time string to mins
data['Total time'] = data['Total time'].astype('str') 
data.replace({'Total time':{'一天':1440,'一小时':60,'三刻钟':45,'十分钟':10,'半小时':30,'廿分钟':20,'数天':4320,'数小时':180,'':-1,'nan':-1}},inplace=True)
data.head()
...
data['Total time'] = data['Total time'].astype(int)
data['Total time'] = data['Total time'].replace(-1, np.nan)
data.head()
...
data.to_csv("/Users/xixi/Dropbox/food4thought/data/final/China.csv")
