...
# #### Variable constructions of Barbados
...
# import dataset
data = pd.read_csv("/Users/xixi/Dropbox/food4thought/data/final/Barbados.csv")
data.head()
...
# ### Drop recipes with barbacoa

# In[6]:


data['barbacoa'] = data['Name of the recipe'].apply(lambda x: "barbacoa" in x.lower())
data.head()
...
def getTime(string):
    """
    input: string in total time
    output: number of mins
    
    """
    if string != string:
        return np.nan
    
    stringLst = [int(s) for s in re.findall(r'\d+', string)]
    
    if len(stringLst) == 1:
        if "M" in string:
            result = stringLst[0]
        elif "H" in string:
            result = stringLst[0]*60
    
    else:
        result = stringLst[0]*60+stringLst[1]
        
    return result

data['Total time'] = data['Total time'].apply(lambda x: getTime(x))
data.head()
...
data.to_csv("/Users/xixi/Dropbox/food4thought/data/final/Barbados.csv")
