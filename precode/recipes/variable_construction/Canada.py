...
# import dataset
data = pd.read_csv("C:/Users/stell/Dropbox/food4thought/analysis23/data/precoded/recipes/intermediate/Canada.csv")
...
# Function to extract total time from instructions
def extract_time_from_instructions(instruction, ingredients):
    total_minutes = 0
    ...
    # Define patterns to capture time expressions like "X minutes" or "Y hours"
    time_patterns = [
        (r'(\d+)\s*minute[s]?', 1),  # Capture minutes
        (r'(\d+)\s*hour[s]?', 60),   # Capture hours and convert to minutes
        (r'(\d+)\s*day[s]?', 1440)   # Capture days and convert to minutes
    ]
    
    for instruction in instructionLst:
        instruction = instruction.lower()
        
        # Apply each pattern to extract and convert time
        for pattern, multiplier in time_patterns:
            matches = re.findall(pattern, instruction)
            if matches:
                total_minutes += sum(int(match) * multiplier for match in matches)
    
    # If total_minutes is 0, fallback to extracting time from ingredients
    if total_minutes == 0:
...
# Apply the function to the 'List of instructions' and 'List of ingredients' columns
data['Total time from instructions'] = data.apply(
    lambda x: extract_time_from_instructions(x['List of instructions'], x['List of ingredients']),
    axis=1
)

# Extract numbers from time columns
data['Cook time'] = data['Cook time'].str.extract('(\d+)').replace(np.nan, 0).astype(int)
data['Prep time'] = data['Prep time'].str.extract('(\d+)').replace(np.nan, 0).astype(int)

# Calculate Total time
data['Total time'] = data['Cook time'] + data['Prep time']

# Replace 0 in Total time with time extracted from instructions
data['Total time'] = np.where(data['Total time'] == 0, data['Total time from instructions'], data['Total time'])

# Replace NaN in Total time with time extracted from instructions
data['Total time'] = data['Total time'].replace(0, np.nan)  # Convert 0s to NaN
data['Total time'] = data['Total time'].fillna(data['Total time from instructions'])
...
# import spice data
spice = pd.read_excel("C:/Users/stell/Dropbox/food4thought/analysis23/data/raw/roster_spices/roster_spices_edited.xlsx", engine='openpyxl', sheet_name="Spices").dropna(how='all')
mixes = pd.read_excel("C:/Users/stell/Dropbox/food4thought/analysis23/data/raw/roster_spices/roster_spices_edited.xlsx", engine='openpyxl', sheet_name="Mixes").dropna(how='all')
...
def unitInLst(unitTagger):
    ...
    # import unit data
    unitData = pd.read_excel("C:/Users/stell/Dropbox/food4thought/analysis23/data/raw/unit_data/roster_unit.xlsx", engine='openpyxl')
...
def sugarAmount(ingredientLstTagger):
    ...
    unitMeasure = pd.read_excel("C:/Users/stell/Dropbox/food4thought/analysis23/data/raw/unit_data/Unit standard.xlsx", engine='openpyxl')
...
data.to_csv("C:/Users/stell/Dropbox/food4thought/analysis23/data/precoded/recipes/final/Canada.csv")
