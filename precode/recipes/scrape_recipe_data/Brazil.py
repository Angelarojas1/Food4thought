...
def htmlOnePageSpider(category_url, dic):
    ...
    headers = {
        ...
        'cookie': 'recipes_bra=b46fc6352d6181ae128ed3e0e2bdbf42; _ga=amp-fxwURsW0k6i4pjjIqIT2eg',
...
def fillBrazilData(key,dic):
    """
    input: key, the key in html dic
    output: fill in data
    
    """
    content = Brazilspider(htmlDic[key])
...
htmlDic = htmlAllPageSpider("https://www.cocina-brasilena.com/recetas",18)
for key in htmlDic:
    try:
        fillBrazilData(key,Brazildata)
    except:
        time.sleep(5)
...
Brazil = pd.DataFrame(Brazildata)
print(Brazil.shape)
Brazil.head()

# save dataset
Brazil.to_csv("/Users/xixi/Dropbox/food4thought/data/raw/Brazil.csv")
