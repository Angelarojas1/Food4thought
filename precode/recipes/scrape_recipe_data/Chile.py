...
def htmlOnePageSpider(category_url, dic):
    ...
    headers = {
        ...
        'cookie': 'recipes_chi=e97e1fd2ede5cd66a8af692aa0e84db6; _ga=amp-4VgQIfGeAuoQEsV9NX_kvAA',
...
def fillChileData(key,dic):
    """
    input: key, the key in html dic
    output: fill in data
    
    """
    content = Chilespider(htmlDic[key])
...
htmlDic = htmlAllPageSpider("https://www.cocina-chilena.com/recetas",41)
for key in htmlDic:
    try:
        fillChileData(key,Chiledata)
    except:
        time.sleep(5)
...
Chile = pd.DataFrame(Chiledata)
print(Chile.shape)
Chile.head()

# save dataset
Chile.to_csv("/Users/xixi/Dropbox/food4thought/data/raw/Chile.csv")
