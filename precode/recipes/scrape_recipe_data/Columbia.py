...
# #### Scrape all recipes of Columbia
# 
...
def htmlOnePageSpider(category_url, dic):
    ...
    headers = {
        ...
        'cookie': 'recipes_col=726aa30b6c376a414422baec42e74a7e; _ga=amp-UutsJQn48_IS0zRQGzHBAg',
...
def Columbiaspider(recipes_url):
    """
...
def fillColumbiaData(key,dic):
    """
    input: key, the key in html dic
    output: fill in data
    
    """
    content = Columbiaspider(htmlDic[key])
...
htmlDic = htmlAllPageSpider("https://www.cocina-colombiana.com/recetas",42)
for key in htmlDic:
    try:
        fillColumbiaData(key,Columbiadata)
    except:
        time.sleep(5)
...
Columbia = pd.DataFrame(Columbiadata)
print(Columbia.shape)
Columbia.head()

# save dataset
Columbia.to_csv("/Users/xixi/Dropbox/food4thought/data/raw/Columbia.csv")
