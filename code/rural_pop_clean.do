   ...
   ** EDITTED BY:       
   ** Last date modified: March 18, 2026

* import FLFP dataset
import delimited "${rawdata}/rural_population/rural_pop.csv", encoding(UTF-8) varnames(1) case(lower) clear
...
save "${mlfp}/rural_pop_long.dta", replace

** keep the most recent year
keep if year == 2019
isid country
unique country
note: There are `r(sum)' countries in rural population data.

** save dataset
save "${pop}/rural_pop_2019.dta", replace
note
