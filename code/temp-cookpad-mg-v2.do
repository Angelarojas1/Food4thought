...
*keep if continent == "Europe"
	...
	*------------------------------------------*
	...
	eststo clear
	forvalue i=6/11{
	eststo m`i':  ivreg2 lfpr (w_mean_spices  = vers_distCapital_3000_std) ${c`i'} i.region_cat i.cl_md i.ym if vers_distCapital_3000 != 0 & covid == 0  & ${s`j'}, partial(i.region_cat i.cl_md i.ym) cluster(adm0)
		qui sum `e(depvar)' if e(sample)
		local m = r(mean)
	estadd scalar Ffirst=e(rkf)
	estadd scalar Mean = `m': m`i'
