...
quietly merge m:1 adm0 using "${versatility}/imported/importbycountry_v3_`z'.dta"
assert _merge !=2
...
*		estadd scalar mean = `mean'
	...
	outreg2 using "${outputs}/Tables/iv_best/cookpad/`var'_v3_nn.xls", lab dec(4) excel par(se) stats(coef se) keep(fem_imp) addstat(f-value, `fval') ctitle("`x'`y'_`z'") nocons title("`var'")
	...
	erase "${outputs}/Tables/iv_best/cookpad/ltime_v3_nn.txt"
	erase "${outputs}/Tables/iv_best/cookpad/ing_v3_nn.txt"
	erase "${outputs}/Tables/iv_best/cookpad/spice_v3_nn.txt"