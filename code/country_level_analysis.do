...
// 	}
	...
	forvalue i=1/5 { 
		eststo: ivreg2 LFP (w_mean_spices i.fem_lfp#c.w_mean_spices = i.fem_lfp#c.vers_distCapital_2000 vers_distCapital_2000) fem_lfp ${c`i'}  ///
	...
	forvalue i=1/5 { 
		eststo: ivreg2 LFP (w_mean_spices i.fem_lfp#c.w_mean_spices = i.fem_lfp#c.vers_distCapital_3000 vers_distCapital_3000) fem_lfp ${c`i'}  ///
	...
	forvalue i=7/10 { 
		eststo: ivreg2 LFP (w_mean_spices i.fem_lfp#c.w_mean_spices = i.fem_lfp#c.vers_distCapital_2000 vers_distCapital_2000) fem_lfp ${c`i'} i.fem_lfp#c.trade_distCapital_2000 ///
		i.region_cat i.cl_md if vers_distCapital_2000 != 0 , robust cluster(adm0)
		qui sum `e(depvar)' if e(sample)
		estadd scalar Mean = r(mean)
	}
	...
	forvalue i=12/15 { 
		eststo: ivreg2 LFP (w_mean_spices i.fem_lfp#c.w_mean_spices = i.fem_lfp#c.vers_distCapital_3000 vers_distCapital_3000) fem_lfp ${c`i'} i.fem_lfp#c.trade_distCapital_3000 ///
		i.region_cat i.cl_md if vers_distCapital_3000 != 0 , robust cluster(adm0)
		qui sum `e(depvar)' if e(sample)
		estadd scalar Mean = r(mean)
	}
	...
	* Export table with F-stat
	estout using reg_gap_IV.tex, ///
		...
		cells(b(star f(3)) se(par f(3))) ///
		starlevels(* 0.10 ** 0.05 *** 0.01) ///
		order(1.fem_lfp#c.w_mean_spices w_mean_spices) ///
...
**#	*   		     Regressions 	 	       *
	...
	* creating the LFP gap at the country-level
	collapse (mean) $c5 LFP_male LFP_female median_spices w_mean_spices vers_distCapital_2000 vers_distCapital_3000 trade_distCapital_2000 trade_distCapital_3000 (first) continent region_cat cl_md, by(adm0)
...
// 	}
	forvalue i=1/5 { 
		eststo: ivreg2 LFP_female (w_mean_spices = vers_distCapital_2000) ${c`i'} i.region_cat i.cl_md if vers_distCapital_2000 != 0, robust first
		qui sum `e(depvar)' if e(sample)
		estadd scalar Mean = r(mean)
	}

	forvalue i=1/5 { 
		eststo: ivreg2 LFP_female (w_mean_spices = vers_distCapital_3000) ${c`i'} i.region_cat i.cl_md if vers_distCapital_3000 != 0, robust first
		qui sum `e(depvar)' if e(sample)
		estadd scalar Mean = r(mean)
	}
	forvalue i=7/10 { 
...
// 	}
	forvalue i=1/5{ 
		eststo: ivreg2 LFP_male (w_mean_spices = vers_distCapital_2000) ${c`i'} i.region_cat i.cl_md if vers_distCapital_2000 != 0, robust first
		qui sum `e(depvar)' if e(sample)
		estadd scalar Mean = r(mean)
	}

	forvalue i=1/5{ 
		eststo: ivreg2 LFP_male (w_mean_spices = vers_distCapital_3000) ${c`i'} i.region_cat i.cl_md if vers_distCapital_3000 != 0, robust first
		qui sum `e(depvar)' if e(sample)
		estadd scalar Mean = r(mean)
	}
